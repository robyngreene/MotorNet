...
class Skeleton(th.nn.Module):
    ...
    def __init__(
        ...
        space_dim: int,
        name: str = "skeleton",
        pos_lower_bound: Union[float, list, tuple] = -1.0,
        pos_upper_bound: Union[float, list, tuple] = +1.0,
        vel_lower_bound: Union[float, list, tuple] = -1000.0,
        vel_upper_bound: Union[float, list, tuple] = +1000.0,
        **kwargs,
    ):
        ...
        self.geometry_state_dim = (
            2 + self.dof
        )  # two geometry variable per muscle: path_length, path_velocity
        self.default_endpoint_load = th.zeros(
            (1, self.space_dim)
        )  # tf.zeros((1, self.space_dim), dtype=tf.float32)
    ...
    def build(
        ...
        self.pos_upper_bound = Parameter(
            th.tensor(pos_upper_bound, dtype=th.float32).reshape(1, -1),
            requires_grad=False,
        )
        self.pos_lower_bound = Parameter(
            th.tensor(pos_lower_bound, dtype=th.float32).reshape(1, -1),
            requires_grad=False,
        )
        self.vel_upper_bound = Parameter(
            th.tensor(vel_upper_bound, dtype=th.float32).reshape(1, -1),
            requires_grad=False,
        )
        self.vel_lower_bound = Parameter(
            th.tensor(vel_lower_bound, dtype=th.float32).reshape(1, -1),
            requires_grad=False,
        )
        self.dt = timestep
    ...
    def clip_velocity(self, pos, vel):
        ...
        vel = th.where(th.logical_and(vel < 0, pos <= self.pos_lower_bound), 0.0, vel)
        vel = th.where(th.logical_and(vel > 0, pos >= self.pos_upper_bound), 0.0, vel)
        return vel
...
class PointMass(Skeleton):
    ...
    def __init__(
        self, space_dim: int = 2, mass: float = 1.0, name: str = "point_mass", **kwargs
    ):
        super().__init__(dof=space_dim, space_dim=space_dim, name=name, **kwargs)
        self.mass = mass
        # to speed up runtime during `self._path2cartesian` method
        dpos_ddof = th.nn.functional.one_hot(th.arange(0, self.dof) % self.dof).to(
            th.float32
        )[None, :, :, None]
    ...
    def _path2cartesian(self, path_coordinates, path_fixation_body, joint_state):
        ...
        pos = th.where(path_fixation_body == 0, 0.0, pos) + path_coordinates
        vel = th.where(path_fixation_body == 0, 0.0, vel)
        dpos_ddof = th.where(path_fixation_body == 0, 0.0, self.dpos_ddof)
...
class TwoDofArm(Skeleton):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        inertia_c = np.array(
            [[[inertia_11_c, inertia_12_c], [inertia_12_c, inertia_22_c]]]
        ).astype(np.float32)
        inertia_m = np.array(
            [[[inertia_11_m, inertia_12_m], [inertia_12_m, 0.0]]]
        ).astype(np.float32)
        self.inertia_m = Parameter(
            th.tensor(inertia_m.reshape((1, 2, 2)), dtype=th.float32),
            requires_grad=False,
        )
        self.inertia_c = Parameter(
            th.tensor(inertia_c.reshape((1, 2, 2)), dtype=th.float32),
            requires_grad=False,
        )
        ...
        self.c_viscosity = (
            viscosity  # put at zero but available if implemented later on
        )
    ...
    def _ode(self, inputs, joint_state, endpoint_load):
        ...
        coriolis_1 = (
            self.coriolis_1 * s2 * (2 * vel0 + vel1)
        ) * vel1 + self.c_viscosity * vel0
        ...
        r_col = (jacobian_11 * endpoint_load[:, 0]) + (
            jacobian_21 * endpoint_load[:, 1]
        )  # these are torques
        l_col = (jacobian_12 * endpoint_load[:, 0]) + (
            jacobian_22 * endpoint_load[:, 1]
        )
        ...
        return new_acc_3d[
            :, :, 0
        ]  # somehow tf.squeeze doesn't work well in a Lambda wrap...
    ...
    def _path2cartesian(self, path_coordinates, path_fixation_body, joint_state):
        ...
        ang = th.where(
            flat_path_fixation_body == 0.0,
            0.0,
            th.where(flat_path_fixation_body == 1.0, -sho, -elb),
        )
        ca = th.cos(ang)
        sa = th.sin(ang)
        ...
        dx_da1 = th.where(path_fixation_body == 0.0, 0.0, dx_da) + th.where(
            path_fixation_body == 2.0, -elb_y, 0.0
        )
        dy_da1 = th.where(path_fixation_body == 0.0, 0.0, dy_da) + th.where(
            path_fixation_body == 2.0, elb_x, 0.0
        )
        dx_da2 = th.where(path_fixation_body == 2.0, dx_da, 0.0)
        dy_da2 = th.where(path_fixation_body == 2.0, dy_da, 0.0)
        ...
        dxy_dt = (
            dxy_da1 * sho_vel_3d + dxy_da2 * elb_vel_3d
        )  # by virtue of the chain rule

        bone_origin = th.where(
            path_fixation_body == 2, th.concat([elb_x, elb_y], dim=1), 0.0
        )
