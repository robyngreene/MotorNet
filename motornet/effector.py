...
class Effector(th.nn.Module):
    ...
    def __init__(
        ...
        self.__name__ = name
        self.damping = Parameter(
            th.tensor(damping, dtype=th.float32), requires_grad=False
        )
        ...
        self.half_minidt = (
            self.minidt / 2
        )  # to reduce online calculations for RK4 integration
        self.integration_method = (
            integration_method.casefold()
        ...
        pos_lower_bound = (
            self.skeleton.pos_lower_bound
            if pos_lower_bound is None
            else pos_lower_bound
        )
        pos_upper_bound = (
            self.skeleton.pos_upper_bound
            if pos_upper_bound is None
            else pos_upper_bound
        )
        vel_lower_bound = (
            self.skeleton.vel_lower_bound
            if vel_lower_bound is None
            else vel_lower_bound
        )
        vel_upper_bound = (
            self.skeleton.vel_upper_bound
            if vel_upper_bound is None
            else vel_upper_bound
        )
        ...
        self.force_index = self.muscle.state_name.index(
            "force"
        )  # column index of muscle state containing output force
        self.MusclePaths = []  # a list of all the muscle paths
        self.n_muscles = 0
        self.input_dim = 0
        self.muscle_name = []
        self.muscle_state_dim = self.muscle.state_dim
        self.geometry_state_dim = (
            2 + self.dof
        )  # musculotendon length & velocity + as many moments as dofs
        ...
        self._path_coordinates = np.empty((1, self.skeleton.space_dim, 0)).astype(
            "float32"
        )
        ...
        self.default_endpoint_load = Parameter(
            th.zeros((1, self.skeleton.space_dim)), requires_grad=False
        )
        self.default_joint_load = Parameter(
            th.zeros((1, self.skeleton.dof)), requires_grad=False
        )
        ...
        if self.integration_method == "euler":
            ...
            "runge-kutta4",
            "runge-kutta-4",
        ):  # tuple faster than set
            self._integrate = self._rungekutta4
        else:
            raise ValueError(
                "Provided integration method not recognized : {}".format(
                    self.integration_method
                )
            )
        ...
        self.states = {
            key: None
            for key in ["joint", "cartesian", "muscle", "geometry", "fingertip"]
        }
    ...
    def add_muscle(
        ...
        **kwargs,
        ...
        path_fixation_body = (
            np.array(path_fixation_body).astype("float32").reshape((1, 1, -1))
        )
        n_points = path_fixation_body.size
        path_coordinates = (
            np.array(path_coordinates).astype("float32").T[np.newaxis, :, :]
        )
        ...
        self._path_fixation_body = np.concatenate(
            [self._path_fixation_body, path_fixation_body], axis=-1
        )
        self._path_coordinates = np.concatenate(
            [self._path_coordinates, path_coordinates], axis=-1
        )
        self._muscle_index = np.hstack(
            [self._muscle_index, np.tile(np.max(self.n_muscles), [n_points])]
        )
        # indexes where the next item is from a different muscle, to indicate when their difference is meaningless
        self._muscle_transitions = (
            np.diff(self._muscle_index.reshape((1, 1, -1))) == 1.0
        )
        ...
        self._row_splits = np.concatenate(
            [
                np.zeros(1),
                np.diff(self._muscle_index).nonzero()[0] + 1,
                n_total_points - 1,
            ]
        )

        self.path_fixation_body = Parameter(
            th.tensor(self._path_fixation_body, dtype=th.float32), requires_grad=False
        )
        self.path_coordinates = Parameter(
            th.tensor(self._path_coordinates, dtype=th.float32), requires_grad=False
        )
        self.muscle_index = Parameter(
            th.tensor(self._muscle_index, dtype=th.float32), requires_grad=False
        )
        self.muscle_transitions = Parameter(
            th.tensor(self._muscle_transitions, dtype=th.bool), requires_grad=False
        )
        self.row_splits = Parameter(
            th.tensor(self._row_splits, dtype=th.float32), requires_grad=False
        )
    ...
    def _get_geometry(self, joint_state):
        ...
        segment_len_cleaned = th.where(self.muscle_transitions, 0.0, segment_len)
        segment_vel_cleaned = th.where(self.muscle_transitions, 0.0, segment_vel)
        segment_mom_cleaned = th.where(self.muscle_transitions, 0.0, segment_moments)
        ...
        musculotendon_len_as_list = [
            th.sum(y, dim=-1)
            for y in segment_len_cleaned.split(self.section_splits, dim=-1)
        ]
        musculotendon_vel_as_list = [
            th.sum(y, dim=-1)
            for y in segment_vel_cleaned.split(self.section_splits, dim=-1)
        ]
        moment_arms_as_list = [
            th.sum(y, dim=-1)
            for y in segment_mom_cleaned.split(self.section_splits, dim=-1)
        ]
    ...
    def _euler(self, action, endpoint_load, joint_load):
        ...
        states = self.integration_step(
            self.minidt, state_derivative=state_derivative, states=states0
        )
        self._set_state(states)
    ...
    def _rungekutta4(self, action, endpoint_load, joint_load):
        states0 = self.states
        k1 = self.ode(
            action, states=states0, endpoint_load=endpoint_load, joint_load=joint_load
        )
        states = self.integration_step(
            self.half_minidt, state_derivative=k1, states=states0
        )
        k2 = self.ode(
            action, states=states, endpoint_load=endpoint_load, joint_load=joint_load
        )
        states = self.integration_step(
            self.half_minidt, state_derivative=k2, states=states
        )
        k3 = self.ode(
            action, states=states, endpoint_load=endpoint_load, joint_load=joint_load
        )
        ...
        k = {
            key: (k1[key] + 2 * (k2[key] + k3[key]) + k4[key]) / 6 for key in k1.keys()
        }
    ...
    def integration_step(self, dt, state_derivative, states):
        ...
        new_states = {
            "muscle": self.muscle.integrate(
                dt, state_derivative["muscle"], states["muscle"], states["geometry"]
            ),
            "joint": self.skeleton.integrate(
    ...
    def ode(self, action, states, endpoint_load, joint_load):
        ...
        generalized_forces = (
            -th.sum(forces * moments, dim=-1) + joint_load - self.damping * joint_vel
        )
    ...
    def draw_random_uniform_states(self, batch_size):
        ...
        rnd = th.tensor(self.np_random.uniform(size=sz), dtype=th.float32).to(
            self.device
        )
    ...
    def _parse_initial_joint_state(self, joint_state, batch_size):
        if joint_state is None:
            ...
            if n_state == self.state_dim:
                ...
                joint0 = self.draw_fixed_states(
                    position=position, velocity=velocity, batch_size=batch_size
                )
            elif n_state == int(self.state_dim / 2):
                joint0 = self.draw_fixed_states(
                    position=joint_state, batch_size=batch_size
                )
            else:
                raise ValueError
    ...
    def draw_fixed_states(self, batch_size, position, velocity=None):
        ...
        pos = (
            position.cpu().detach().numpy()
            if th.is_tensor(position)
            else np.array(position)
        )
        vel = (
            velocity.cpu().detach().numpy()
            if th.is_tensor(velocity)
    ...
    def _set_state_limit_bounds(self, lb, ub):
        lb = (
            np.array(lb).reshape((-1, 1)).astype(np.float32)
        )  # ensure this is a 2D array
        ub = np.array(ub).reshape((-1, 1)).astype(np.float32)
        bounds = np.hstack((lb, ub))
        bounds = bounds * np.ones((self.dof, 2)).astype(
            np.float32
        )  # if one bound pair, broadcast to dof rows
        return bounds
    ...
    def _merge_muscle_kwargs(self, muscle_kwargs: dict):
        ...
        for key, val in muscle_kwargs.items():
            if key in self.tobuild__muscle.keys():
                ...
                raise KeyError(
                    'Unexpected key "' + key + '" in muscle_kwargs argument.'
                )
...
class ReluPointMass24(Effector):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            skeleton=skeleton, muscle=ReluMuscle(), timestep=timestep, **kwargs
        )
...
class RigidTendonArm26(Effector):
    ...
    def __init__(
        ...
        a1 = [-0.03, 0.03, 0, 0, -0.03, 0.03, 0, 0, -0.014, 0.025, -0.016, 0.03]
        a2 = [0, 0, 0, 0, 0, 0, 0, 0, -4e-3, -2.2e-3, -5.7e-3, -3.2e-3]
        a3 = [np.pi / 2, 0.0]
        self.a0 = Parameter(
            th.tensor(np.array(a0).reshape((1, 1, 6)), dtype=th.float32),
            requires_grad=False,
        )
        self.a1 = Parameter(
            th.tensor(np.array(a1).reshape((1, 2, 6)), dtype=th.float32),
            requires_grad=False,
        )
        self.a2 = Parameter(
            th.tensor(np.array(a2).reshape((1, 2, 6)), dtype=th.float32),
            requires_grad=False,
        )
        self.a3 = Parameter(
            th.tensor(np.array(a3).reshape((1, 2, 1)), dtype=th.float32),
            requires_grad=False,
        )
    ...
    def _get_geometry(self, joint_state):
        ...
        musculotendon_len = (
            th.sum((self.a1 + old_pos * self.a2) * old_pos, dim=1, keepdims=True)
            + self.a0
        )
