...
from IPython.display import (
    HTML,
    display_html,
)  # TODO: make sure this works outwith notebooks
...
def _results_to_line_collection(results):
    ...
    # n_lines * 2 * space_dim
    segments_all_batches = np.moveaxis(segments_by_batch, -1, 0).reshape(
        (-1, 2, space_dim)
    )
...
def plot_arm_location(joint_state, arm, axis, linewidth=1, cmap="viridis"):
    ...
    segments = np.squeeze(np.concatenate([upper_arm, lower_arm], axis=0))
    _plot_line_collection(
        axis, segments, cmap=cmap, linewidth=linewidth, n_gradient=n_timesteps
    )
...
def animate_2dof_arm_over_time(
    arm, joint_state, tg=None, cmap: str = "viridis", linewidth: int = 1
):
    """Plots an arm26 over time, with earlier and later arm configuration in the movement being represented as darker
    and brighter colors, respectively.
    ...
    def animate(frame_number, lc):
        ...
        segments_i = np.squeeze(
            np.concatenate(
                [
                    segments_u[frame_number : frame_number + 1, :, :],
                    segments_l[frame_number : frame_number + 1, :, :],
                ],
                axis=0,
            )
        )
