...
class Muscle(th.nn.Module):
    ...
    def __init__(
        ...
        self.min_activation = Parameter(
            th.tensor(min_activation, dtype=th.float32), requires_grad=False
        )
        self.tau_activation = Parameter(
            th.tensor(tau_activation, dtype=th.float32), requires_grad=False
        )
        self.tau_deactivation = Parameter(
            th.tensor(tau_deactivation, dtype=th.float32), requires_grad=False
        )
    ...
    def clip_activation(self, a):
        return th.clamp(a, self.min_activation, 1.0)
    ...
    def build(self, timestep, max_isometric_force, **kwargs):
        ...
        self.max_iso_force = Parameter(
            th.tensor(max_isometric_force, dtype=th.float32), requires_grad=False
        )
        self.dt = timestep
...
class ReluMuscle(Muscle):
    ...
    def _get_initial_muscle_state(self, batch_size, geometry_state):
        activation0 = (
            th.ones((batch_size, 1, self.n_muscles), device=self.device)
            * self.min_activation
        )
...
class MujocoHillMuscle(Muscle):
    ...
    def build(
        ...
        self.max_iso_force = Parameter(
            to_tensor(max_isometric_force), requires_grad=False
        )
        self.l0_pe = Parameter(
            to_tensor(normalized_slack_muscle_length), requires_grad=False
        )
        ...
        self.zero_as_tensor = Parameter(
            th.tensor(0.0, dtype=th.float32), requires_grad=False
        )
    ...
    def _get_initial_muscle_state(self, batch_size, geometry_state):
        ...
        muscle_state = (
            th.ones(shape, dtype=th.float32, device=self.device) * self.min_activation
        )
    ...
    def _bump(self, L, mid, lmax):
        ...
        out_of_range = th.logical_or(
            th.less_equal(L, self.lmin), th.greater_equal(L, lmax)
        )
        ...
        x = th.where(
            ...
            other=th.where(
                less_than_left,
                input=(L - self.lmin) / (left - self.lmin),
                other=th.where(
                    less_than_mid,
                    input=(mid - L) / (mid - left),
                    other=th.where(
                        less_than_right,
                        input=(L - mid) / (right - mid),
...
class RigidTendonHillMuscle(Muscle):
    ...
    def build(
        ...
        self.dt = timestep
        self.max_iso_force = Parameter(
            th.tensor(max_isometric_force, dtype=th.float32).reshape(shape),
            requires_grad=False,
        )
        self.l0_ce = Parameter(
            th.tensor(optimal_muscle_length, dtype=th.float32).reshape(shape),
            requires_grad=False,
        )
        self.l0_se = Parameter(
            th.tensor(tendon_length, dtype=th.float32).reshape(shape),
            requires_grad=False,
        )
        self.l0_pe = Parameter(
            th.tensor(normalized_slack_muscle_length, dtype=th.float32) * self.l0_ce,
            requires_grad=False,
        )
        self.k_pe = Parameter(
            1 / ((1.66 - self.l0_pe / self.l0_ce) ** 2), requires_grad=False
        )
        self.musculotendon_slack_len = Parameter(
            self.l0_pe + self.l0_se, requires_grad=False
        )
    ...
    def _integrate(self, dt, state_derivative, muscle_state, geometry_state):
        ...
        muscle_len = th.clip(musculotendon_len - self.l0_se, min=0.0)
        muscle_strain = th.clip((muscle_len - self.l0_pe) / self.l0_ce, min=0.0)
        ...
        a_rel_st = th.where(condition=muscle_len_n > 1.0, input=0.41 * flce, other=0.41)
        b_rel_st = th.where(
            condition=activation < self.q_crit,
            input=5.2
            * (1 - 0.9 * ((activation - self.q_crit) / (5e-3 - self.q_crit))) ** 2,
            other=5.2,
        )
        dfdvcon0 = (
            activation * (flce + a_rel_st) / b_rel_st
        ...
        active_force = th.clip(nom / den, min=0.0)
        force = (active_force + flpe) * self.max_iso_force
        return th.cat(
            [activation, muscle_len, muscle_vel, flpe, flce, active_force, force], dim=1
        )
...
class RigidTendonHillMuscleThelen(Muscle):
    ...
    def __init__(self, min_activation=0.001, **kwargs):
        ...
        # parameters for the passive element (PE) and contractile element (CE)
        self.pe_k = Parameter(th.tensor(5.0, dtype=th.float32), requires_grad=False)
        self.pe_1 = Parameter(
            self.pe_k / 0.6, requires_grad=False
        )  # divided by epsilon_0^M in Thelen (2003) eq. 3
        self.pe_den = Parameter(th.exp(self.pe_k) - 1, requires_grad=False)
        self.ce_gamma = Parameter(
            th.tensor(0.45, dtype=th.float32), requires_grad=False
        )
    ...
    def build(
        ...
        max_isometric_force = th.tensor(max_isometric_force, dtype=th.float32).reshape(
            1, 1, self.n_muscles
        )
        optimal_muscle_length = th.tensor(
            optimal_muscle_length, dtype=th.float32
        ).reshape(1, 1, self.n_muscles)
        tendon_length = th.tensor(tendon_length, dtype=th.float32).reshape(
            1, 1, self.n_muscles
        )
        normalized_slack_muscle_length = th.tensor(
            normalized_slack_muscle_length, dtype=th.float32
        )
        ...
        self.l0_pe = Parameter(
            self.l0_ce * normalized_slack_muscle_length, requires_grad=False
        )
        self.musculotendon_slack_len = Parameter(
            self.l0_pe + self.l0_se, requires_grad=False
        )
        ...
        self.ce_3 = Parameter(
            8 * self.ce_Af * self.ce_fmlen + 8.0 * self.ce_fmlen, requires_grad=False
        )
        self.ce_4 = Parameter(
            self.ce_Af * self.ce_fmlen * self.vmax - self.ce_1, requires_grad=False
        )
        self.ce_5 = Parameter(8 * (self.ce_Af + 1.0), requires_grad=False)
    ...
    def _integrate(self, dt, state_derivative, muscle_state, geometry_state):
        ...
        nom = th.where(
            condition,
            input=self.ce_Af * (activation * self.ce_0 + 4.0 * muscle_vel + self.vmax),
            other=self.ce_2 * activation + self.ce_3 * muscle_vel + self.ce_4,
        )
        den = th.where(
            condition,
            input=a3 * self.ce_1 + self.ce_1 - 4.0 * muscle_vel,
            other=self.ce_4 * a3 + self.ce_5 * muscle_vel + self.ce_4,
        )
        fvce = th.clip(nom / den, min=0.0)
        flpe = th.clip(
            (th.exp(self.pe_1 * (muscle_len - self.l0_pe) / self.l0_ce) - 1)
            / self.pe_den,
            min=0.0,
        )
...
class CompliantTendonHillMuscle(RigidTendonHillMuscle):
    ...
    def _integrate(self, dt, state_derivative, muscle_state, geometry_state):
        ...
        tendon_strain = th.clip((tendon_len - self.l0_se) / self.l0_se, min=0.0)
        muscle_strain = th.clip((muscle_len - self.l0_pe) / self.l0_ce, min=0.0)

        # Compute forces
        flse = th.clip(self.k_se * (tendon_strain**2), max=1.0)
        # flpe = tf.minimum(self.k_pe * (muscle_strain ** 2), 1.)
        flpe = self.k_pe * (muscle_strain**2)
        active_force = th.clip(flse - flpe, min=0.0)
    ...
    def _get_initial_muscle_state(self, batch_size, geometry_state):
        ...
        activation = (
            th.ones_like(musculotendon_len, device=self.device) * self.min_activation
        )
        ...
        muscle_len = th.where(
            condition=th.less(musculotendon_len, 0.0),
            input=th.tensor(-1.0, dtype=th.float32, device=self.device),
            other=th.where(
        ...
        # Compute forces
        flse = th.clip(self.k_se * (tendon_strain**2), max=1.0)
        flpe = th.clip(self.k_pe * (muscle_strain**2), max=1.0)
        active_force = th.clip(flse - flpe, min=0.0)
    ...
    def _normalized_muscle_vel(self, muscle_len_n, activation, active_force):
        flce = th.clip(
            1.0 + (-(muscle_len_n**2) + 2 * muscle_len_n - 1) / self.f_iso_n_den,
            min=self.min_flce,
        )
        a_rel_st = th.where(th.less(muscle_len_n, 1.0), input=0.41 * flce, other=0.41)
        b_rel_st = th.where(
        ...
        sqrt_term = th.clip(sqrt_term, min=0.0)
