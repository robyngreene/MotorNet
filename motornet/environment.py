...
class Environment(gym.Env, th.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def floating_precision(x):
            return x < np.finfo(x).resolution

        assert floating_precision(
            np.mod(vision_delay / self.dt, 1.0)
        ), f"delay was {vision_delay} and dt was {self.dt}"
        assert floating_precision(np.mod(proprioception_delay / self.dt, 1.0)), (
            f"delay was {proprioception_delay} and " + f"dt was {self.dt}"
        )
    ...
    def _build_spaces(self):
        self.action_space = gym.spaces.Box(
            low=0.0, high=1.0, shape=(self.effector.n_muscles,), dtype=np.float32
        )
    ...
    def get_obs(
        self, action=None, deterministic: bool = False
    ...
    def update_obs_buffer(self, action=None):
        ...
        if action is not None:
            self.obs_buffer["action"] = self.obs_buffer["action"][1:] + [
                action.reshape(-1, self.action_space.shape[0])
            ]
    ...
    def _maybe_detach_states(self):
        return (
            ...
            else {key: self.detach(val) for key, val in self.states.items()}
        )
    ...
    def apply_noise(self, loc, noise: float | list) -> th.Tensor:
        ...
        """
        white_noise = self.np_random.normal(
            size=(loc.shape[0], len(noise)), scale=noise
        )
    ...
    def get_attributes(self):
        ...
        attributes = [
            ...
            and not isinstance(
                getattr(self, a), gym.spaces.Space
            )  # Spaces are not JSON serializable
        ]
...
class CentreOutReach(Environment):
    ...
    def reset(
        ...
        self.elapsed = 0.0  # track time since befinning of episode. Reset to 0 here
...
class OneDimensionalReach(Environment):
    ...
    def reset(
        ...
        # TODO: based on angles required for fingertip to start at centre of circle. could be a function.
        sho_deg, elb_deg = find_shoulder_elbow_angles_for_coord(
            arm=self.effector, y=self.lift_height
        )
        # sho_deg = 90-66.278
        # elb_deg = 180 - 55.565
        sho_rad = np.radians(sho_deg)
...
class MultiTaskReach(Environment):
    ...
    def reset(
        ...
        if task_name is not None:
            ...
            assert task_name in [
                task.__name__ for task in self.tasks
            ], "Invalid task name. Valid names are: " + ", ".join(task_names)
